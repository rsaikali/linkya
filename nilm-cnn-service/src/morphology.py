...
class MorphologyAnalyzer:
    ...
    def analyze(
        ...
        try:
            analysis = {
                ...
                'frequency_domain': self._compute_frequency_features(
                    power_values
                ),
            }
    ...
    def _classify_pattern(self, power_norm: np.ndarray) -> str:
        ...
        if np.mean(gradient[gradient > 0]) > 0.01 and np.sum(
            gradient > 0
        ) > len(gradient) * 0.7:
            return 'ramp_up'
    ...
    def _compute_gradient_features(
    ...
    def _compute_oscillation_features(
        self, power: np.ndarray
    ) -> Dict[str, Any]:
        ...
        if len(peaks) > 1:
            ...
            avg_freq = 1.0 / np.mean(intervals) if np.mean(
                intervals
            ) > 0 else 0
        else:
            regularity = 0.0
        ...
        return {
            ...
            'avg_peak_amplitude': float(
                np.mean(properties.get('prominences', [0]))
            ),
        }
    
    def _compute_statistical_moments(
        self, power: np.ndarray
    ) -> Dict[str, float]:
