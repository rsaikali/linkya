# AGENTS.md

This file acts as a guide when working with code in this repository.
Keep it simple: no over-engineering in your actions, but try to be the expert you are in every domain asked.
Don't hesitate to research best practices if needed.

When asked for a modification, explain your implementation choices to the user briefly.
Ask the user if any information is missing.
Ask which parts to prioritize if relevant.
After implementation, always test your changes thoroughly before submitting.

## Context Documentation
The "docs/DOC.md" file contains everything you need to know about the project to quickly get a good understanding as an expert coding assistant.
Create the "docs/DOC.md" file, populate it with relevant information and update it periodically as the project evolves.
In this file, do not be too detailed to avoid overloading your context, but be precise enough to help you understand the whole application quickly.
Keep only the essentials such as the overall architecture and structure of the project, the main technologies used, and any specific conventions or patterns followed in the codebase.

## Coding Standards
The environment to use is defined in the root .env file.
Everything in the codebase must be in English language.
For Python, follow PEP 8 guidelines.
For JavaScript/TypeScript, follow Airbnb style guide.
No emojis in the codebase, neither in logs.
Do not create unnecessary documentation or markdown files to report your actions.
Do not create unnecessary scripts, or delete them once they are no longer needed.
When modifying a service, ensure that all related services are also updated accordingly.