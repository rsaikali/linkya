...
const SignaturesTimeline = ({ rawData, signatures }) => {
  ...
  // Prepare annotations for signatures
  const annotationsData = useMemo(() => {
    if (!rawData || !signatures || signatures.length === 0) {
      return {};
    }
    ...
    return annotations;
  }, [rawData, signatures, getApplianceColor]);
  ...
  useEffect(() => {
    ...
    const handleTooltipMove = (e) => {
      ...
      if (dataIndex >= 0 && dataIndex < rawData.data.length) {
        ...
        const hoveredSignature = signatures.find(s => {
          const startTime = new Date(s.start_time).getTime();
          const endTime = new Date(s.end_time).getTime();
        ...
        if (hoveredSignature) {
          const startDate = new Date(hoveredSignature.start_time);
          const endDate = new Date(hoveredSignature.end_time);
          ...
          setCustomTooltip({
            ...
            y: e.clientY - 10,
            content: {
              name: hoveredSignature.appliance_name,
              startTime: startDate.toLocaleString('fr-FR'),
              endTime: endDate.toLocaleString('fr-FR'),
              duration: duration.toFixed(1),
              energy: hoveredSignature.energy_consumed?.toFixed(2) || 'N/A',
              isNegative: hoveredSignature.is_negative === true,
            },
          });
        } else {
  ...
  return (
    <>
      <Box sx={{ height: 100, position: 'relative', borderTop: '1px solid rgba(0,0,0,0.1)' }}>
        <Typography 
          variant="caption" 
          sx={{ 
            position: 'absolute', 
