...
async def delete_all_detections():
    ...
    try:
        ...
        if redis_client:
            try:
                message = json.dumps(
                    {"event": "detections_cleared", "data": {"deleted_count": result["deleted_count"]}, "timestamp": datetime.utcnow().isoformat()}
                )
...
async def validate_detection(detection_id):
    ...
    try:
        ...
        logger.error(f"Error validating detection {detection_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Erreur serveur: {str(e)}")
...
async def invalidate_detection(detection_id):
    ...
    try:
        ...
        logger.error(f"Error invalidating detection {detection_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Erreur serveur: {str(e)}")
...
async def reassign_detection(detection_id, request):
    ...
    try:
        ...
        logger.error(f"Error reassigning detection " f"{detection_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Erreur serveur: {str(e)}")
