...
from ..models import ApplianceUpdate, HaPublishUpdate
...
async def update_appliance(appliance_id: int, appliance_data: ApplianceUpdate):
    """Updates an appliance name."""
    try:
        updated_appliance = db_manager.update_appliance(appliance_id=appliance_id, name=appliance_data.name)
        if not updated_appliance:
            raise HTTPException(status_code=404, detail=f"Appliance {appliance_id} not found")
        bus.publish("appliance_updated", {"id": appliance_id, "name": appliance_data.name})
